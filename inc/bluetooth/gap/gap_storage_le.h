...
#define LE_KEY_STORE_REMOTE_SRV_CHANGE_AWARE_STATE_BIT     0x0200 /**< Save remote service change aware state to flash. */
 ...
 * @return Pointer to the found key entry.
 * @retval null No entry found.
 * @retval others Pointer to the found key entry.
 */
 ...
 * @param[out]      p_data        Pointer to CCCD data to read.
 * @return Operation result.
 * @retval true     Operation success.
 * @retval false    Operation failure.
 *
...
/**
 * @brief   Erase CCCD except for a specific attribute handle.
 *
 * @param[in] p_entry        Pointer to the key entry of the bonded device @ref T_LE_KEY_ENTRY.
