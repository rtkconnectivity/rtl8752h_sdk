...
# $1 is the prefix of Git Tag, and default is "bee3plus-sdk-v"
if [ ! $1 ]; then
    TAG_HEADER="bee3plus-sdk-v"
else
    TAG_HEADER=$1v
fi
TAG_HEADER_LEN=`echo $TAG_HEADER | wc -c`
#echo $TAG_HEADER       #bee3plus-sdk-v
...
# SDK: using tag number as revision,not use count number
#BUILDNUM=`echo $PROJ_VERSION | awk -F '-' '{print $2}'`

BUILDNUM=`echo $TAG_VERSION | awk -F '.' '{print $4}'`
BUILDNUM=${BUILDNUM%%-*}

if [ ! $BUILDNUM ]; then
    BUILDNUM=0
fi
if [ $BUILDNUM -gt 31 ]; then
