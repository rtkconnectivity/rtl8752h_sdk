...
{
    UART_SendData(UART2, (uint8_t *)&ch, 1);
    /* wait tx fifo empty */
    while (UART_GetFlagState(UART2, UART_FLAG_THR_TSR_EMPTY) != SET);
...
void UART2_Handler(void)
{
    ...
    /* read interrupt id */
    interrupt_id = UART_GetIID(UART2);

    /* disable interrupt */
    UART_INTConfig(UART2, UART_INT_RD_AVA | UART_INT_LINE_STS, DISABLE);
    ...
    case UART_INT_ID_RX_LEVEL_REACH:
        UART_ReceiveData(UART2, &rx_char, 1);
    ...
    UART_INTConfig(UART2, UART_INT_RD_AVA, ENABLE);
...
void data_uart_init(void *event_queue_handle, void *io_queue_handle)
{
    ...
    Pinmux_Config(DATA_UART_TX_PIN, UART2_TX);
    Pinmux_Config(DATA_UART_RX_PIN, UART2_RX);
