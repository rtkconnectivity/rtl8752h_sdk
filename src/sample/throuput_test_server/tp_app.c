    ...
    if (gap_dev_state.gap_init_state != new_state.gap_init_state)
    {
        if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY)
        {
            ...
            data_uart_print("local bd addr: 0x%2x:%2x:%2x:%2x:%2x:%2x\r\n",
                            ...
                            bt_addr[0]);
            le_adv_start();
            data_uart_print("Start advertising\r\n");
        }
    }
    ...
    case GAP_CONN_PARAM_UPDATE_STATUS_SUCCESS:
        {
            ...
            para_mode = 3;
            update_conn_para_callback(conn_id);
        }
        break;
    ...
    case GAP_MSG_LE_SET_DATA_LEN:
        {
            ...
            if (p_data->p_le_set_data_len_rsp->cause == 0x011a) // for unsport data length extension phone
            {
                leng_mode = 1;
            }
            else
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    if (service_id == SERVICE_PROFILE_GENERAL_ID)
    {
        ...
        case PROFILE_EVT_SEND_DATA_COMPLETE:
            ...
            {
                tp_update_complete(p_param->event_data.send_data_result.conn_id,
                                   p_param->event_data.send_data_result.credits);
            }
            else
            {
        ...
        case VENDOR_TP_SERVICE_CHAR_DT_RX_INDEX:
            {
                ...
                uint8_t *p_value = p_callback_data->write_data.p_value;
                uint16_t length = p_callback_data->write_data.value_len;
                tp_recv_dt_rx(conn_id, p_value, length);
            }
            break;
