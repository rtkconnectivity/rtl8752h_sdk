...
*****************************************************************************************
   * @file      main.c
   * @brief     Source file for Throughput Client project, mainly used for initialize modules
   * @author    jane
   * @date      2017-06-12
   * @version   v1.0
   **************************************************************************************
...
/** @defgroup  THROUPUT_TEST_CLIENT_DEMO Throuput Test Client Main
...
void app_le_gap_init(void)
{
    ...
    uint8_t  device_name[GAP_DEVICE_NAME_LEN] = "THROUPUT_TEST_CLIENT";
...
void app_le_profile_init(void)
{
    ...
    tp_client_id = tp_client_add(app_client_callback);
}
...
void board_init(void)
{
    ...
    APP_PRINT_INFO0("Log Off Sucess.");
#endif
}
 ...
 * @note     There is only one task in Throughput Client APP, thus only one APP task is init here
 * @return   void
 */
void task_init(void)
{
...
int main(void)
{
#if (SUPPORT_FLASH_2_BIT == 1)
    if (FLASH_NOR_RET_SUCCESS == flash_nor_try_high_speed_mode(FLASH_NOR_IDX_SPIC0,
                                                               FLASH_NOR_2_BIT_MODE))
    {
        APP_PRINT_INFO0("flash 2 bit mode switch succeed");
    }
#elif (SUPPORT_FLASH_4_BIT == 1)
    if (FLASH_NOR_RET_SUCCESS == flash_nor_try_high_speed_mode(FLASH_NOR_IDX_SPIC0,
                                                               FLASH_NOR_4_BIT_MODE))
    {
        APP_PRINT_INFO0("flash 4 bit mode switch succeed");
    }
#endif
...
/** @} */ /* End of group THROUPUT_TEST_CLIENT_DEMO_MAIN */
