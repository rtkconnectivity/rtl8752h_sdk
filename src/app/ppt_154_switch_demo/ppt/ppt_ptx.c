...
#define CONTINUOUS_SEND_PKT_NUM     3

static volatile bool g_ppt_initialized = false;

static void ppt_send_complete_msg_to_apptask(void)
{
    static uint8_t recv_cnt = 0;
    if (recv_cnt < CONTINUOUS_SEND_PKT_NUM)
    {
        recv_cnt ++;
        return;
    }
    ...
    bee_msg.subtype = IO_MSG_MPM_PPT;
    ...
    if (reg_misr.tx_int)
    {
        ...
        if (ppt_ctx->ack == false)
        {
            if (ppt_ctx->retransmit_counter == 0)
            {
                ...
                ppt_send_complete_msg_to_apptask();
            }
            else
            {
...
bool ppt_hw_is_idle(void)
{
    PRO_DSM_READ_TYPE dsm = {.d16 = RD_PPT_REG(PRO_DSM_READ)};
    PRO_DSM_READ1_TYPE dsm1 = {.d16 = RD_PPT_REG(PRO_DSM_READ1)};

    APP_PRINT_INFO2("dsm 0x%04x dsm1 0x%04x", dsm.d16, dsm1.d16);
    if (dsm.prx_fsm_idle && dsm.radio_idle && dsm.timing_idle && dsm1.ptx_fsm_idle)
    {
        return true;
    }
    else
...
void ppt_deinit_demo(void)
{
    if (g_ppt_initialized)
    {
        while (ppt_hw_is_idle() == false)
        {

        }
        ppt_deinit();
        ppt_dlps_deinit(); /* ppt_dlps_deinit must be put at the last step of 2.4g actions */
        g_ppt_initialized = false;
    }
}
...
void ppt_demo(void)
{
    if (g_ppt_initialized == false)
    {
        ppt_cfg();
        ppt_reg_handler(ppt_isr_handler);
        g_ppt_initialized = true;
    }
    ...
    while (1)
    {
        ...
        if (++counter == CONTINUOUS_SEND_PKT_NUM)
        {
            break;
        }
    }
