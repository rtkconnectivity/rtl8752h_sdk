    ...
    case BLE_SERVICE_CHAR_DEVICE_INFO_INDEX:
        {
            ...
            DFU_PRINT_INFO3("<==ota_attr_read_cb: ic_type=0x%x, spec_version=0x%x, OTA Header secure_version=0x%x",
            ...
            DFU_PRINT_INFO5("<==ota_attr_read_cb: bufChkEn=%d, bufsize=0x%x, aesflg=%d, aesmode=%d, handshake=%d",
            ...
            if (is_ota_support_bank_switch())
            {
                ...
                if (ota_bank0_addr == get_active_ota_bank_addr())
                {
                    ...
                    DeviceInfo.temp_bank_size = flash_nor_get_bank_size(FLASH_OTA_BANK_1) / FLASH_NOR_SECTOR_SIZE;
                }
                else
                {
                    /* meaningful only when multi_img = 1 */
                    DeviceInfo.temp_bank_size = flash_nor_get_bank_size(FLASH_OTA_BANK_0) / FLASH_NOR_SECTOR_SIZE;
                }
            }
            else
            {
...
#endif
                /* meaningful only when multi_img = 1 */
                DeviceInfo.temp_bank_size = flash_nor_get_bank_size(FLASH_OTA_TMP) / FLASH_NOR_SECTOR_SIZE;
            }
            ...
            DFU_PRINT_INFO3("<==ota_attr_read_cb: image_crtl_header_offset 0x%x, compressed_image_crtl_header_offset 0x%x, compress_mode %d",
    ...
    case BLE_SERVICE_CHAR_IMAGE_VERSION2_INDEX:
        {
            if (ota_active_bank_info.image_num > 10)
            {
                ...
                *p_length = 4 * (ota_active_bank_info.image_num - 10);
                ...
                DFU_PRINT_INFO2("Image Version[10-11]: 0x%x, 0x%x", image_version[10], image_version[11]);
            }
            else
            {
                *pp_value = NULL;
