 ...
 *                              Local Functions
 *============================================================================*/
__attribute__((weak)) bool dfu_soc_check_img_version(uint16_t image_id, uint32_t version)
{
    return true;
}
        ...
        g_dfu_para.cur_offset + g_ota_tmp_buf_used_size == g_dfu_para.image_total_length)
    {
        ...
        else //crc ok
        {
            ...
            if (result == 0)
            {
                ...
                if ((g_dfu_para.cur_offset - dfu_resend_offset) >= FLASH_NOR_SECTOR_SIZE)
                {
            ...
            else
            {
                ...
                if ((g_dfu_para.cur_offset - dfu_resend_offset) > FLASH_NOR_SECTOR_SIZE) //need erase two sector
                {
...
void dfu_service_handle_active_image(void)
{
    ...
    if (!is_ota_support_bank_switch())
    {
        ...
        {
            uint32_t base_addr = dfu_get_temp_ota_bank_addr_by_img_id(temp_image_info[i].image_id);
            p_header = (T_IMG_HEADER_FORMAT *)(base_addr + temp_image_info[i].image_offset);
            if (p_header)
            {
    ...
    else
    {
        ...
        DFU_PRINT_INFO1("==>dfu_service_handle_active_image: bitmap 0x%x", valid_bitmap);
        //support bank switch
        for (uint16_t img_id = IMAGE_MAX - 1; img_id >= OTA; img_id--)
        {
            if (get_valid_bitmap((T_IMG_ID)img_id))
            {
                /* check OTA temp to see if received image is OK.*/
                p_header = (T_IMG_HEADER_FORMAT *)dfu_get_temp_ota_bank_addr_by_img_id((T_IMG_ID)img_id);
                dfu_set_image_ready(p_header, false);
            }
        }
    }

    for (uint16_t img_id = IMAGE_USER_DATA_FIRST; img_id < IMAGE_USER_DATA_MAX; img_id++)
    {
        if (get_valid_bitmap((T_IMG_ID)img_id))
        {
            /* check user data to see if received image is OK.*/
            p_header = (T_IMG_HEADER_FORMAT *)dfu_get_temp_ota_bank_addr_by_img_id((T_IMG_ID)img_id);
            dfu_set_image_ready(p_header, false);
        }
    }
                                          ...
                                          uint8_t *p_value)
{
    ...
    case DFU_OPCODE_START_DFU: //0x01
        if (length == DFU_LENGTH_START_DFU)/*4 bytes is padding for encrypt*/
        {
            ...
            dfu_compressed_mode = (g_dfu_para.start_dfu_mode == START_DFU_COMPRESSED_MODE_VALUE);
            DFU_PRINT_INFO3("DFU_OPCODE_START_DFU: image_total_length=%d, start_dfu_mode=0x%x(%d: compressed mode, others: default mode)",
                            g_dfu_para.image_total_length, g_dfu_para.start_dfu_mode, START_DFU_COMPRESSED_MODE_VALUE);
            if (dfu_compressed_mode)
            {
                g_dfu_para.image_total_length = g_dfu_para.ctrl_header.payload_len + DECODE_OFFSET;
            }
            else
            {
...
//                          temp_image_info[temp_image_num].image_size = UP_ALIGN(g_dfu_para.image_total_length, FMC_SEC_SECTION_LEN);
                            ...
                            temp_image_info[temp_image_num].dfu_compressed_mode = dfu_compressed_mode;
                            if (temp_image_num == 0)
                            {
                        ...
                        DFU_PRINT_INFO5("DFU_OPCODE_START_DFU: image_num=%d, image_id=0x%x, image_size=0x%x, image_offset=0x%x, compressed_mode %d",
                                        ...
                                        temp_image_info[temp_image_num].dfu_compressed_mode);
                    }
    ...
    case DFU_OPCODE_VALID_FW://0x03
        ...
        if (length == DFU_LENGTH_VALID_FW)
        {
            ...
            */
#if (AON_WDG_ENABLE == 1)
            bool aon_wdg_en = AON_WDG_IsEnable();
            if (aon_wdg_en)
            {
                aon_wdg_disable();
            }
    ...
    case DFU_OPCODE_SYSTEM_RESET://0x05
        {
            ...
            if (g_dfu_para.ctrl_header.image_id >= OTA && g_dfu_para.ctrl_header.image_id < IMAGE_MAX)
            {
                ...
                if (p_temp_header && !p_temp_header->ctrl_header.ctrl_flag.flag_value.not_ready)
                {
                    flash_nor_erase_locked(temp_addr, FLASH_NOR_ERASE_SECTOR);
                }
            }
    ...
    case DFU_OPCODE_CHECK_IMAGE_VER://0x0d
        {
            if (length == DFU_LENGTH_IMAGE_VERSION)
            {
                ...
                DFU_PRINT_INFO2("DFU_OPCODE_CHECK_IMAGE_VER: img_id 0x%x, version %x", image_id, version);

                if (dfu_soc_check_img_version(image_id, version))
                {
                    DFU_PRINT_INFO0("DFU_OPCODE_CHECK_IMAGE_VER length version check success!!");
                    notif_data[2] =  DFU_ARV_SUCCESS ;
