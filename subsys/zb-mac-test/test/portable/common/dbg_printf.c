/**************************************************************************//**
 * @file     dbg_printf.c
 * @brief    Source file for debug printf implementation used to output formatted log messages.
...
uint8_t dbg_output_interface = DBG_OUT_IF_NONE;
...
// external platform-specific character output function
...
{
#if TEST_USB_CDC_ACM
    dbg_output_interface = DBG_OUT_IF_USB;
#else
    dbg_output_interface = DBG_OUT_IF_UART;
#endif
}
...
#if TEST_USB_CDC_ACM
static char g_str_format_buf[512];
#else
static char g_str_format_buf[4];
#endif
...
int dbg_printf(const char *format, ...)
{
    ...
    if (dbg_output_interface == DBG_OUT_IF_NONE)
    {
        return 0;
    }
    ...
    if (dbg_output_interface == DBG_OUT_IF_USB)
    {
        len = _vsnprintf(_out_buffer, g_str_format_buf, 512, format, va);
    }
    else
    {
        len = _vsnprintf(_out_char, g_str_format_buf, (size_t) - 1, format, va);
    }
    va_end(va);

    if (dbg_output_interface == DBG_OUT_IF_USB)
    {
        usb_cdc_acm_send((const uint8_t *)g_str_format_buf, len);
    }
    return len;
}
