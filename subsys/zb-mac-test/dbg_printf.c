...
#define GDMA_Channel_VECTORn            GDMA0_Channel0_VECTORn
#define GDMA_BUFFER_SIZE                512

uint8_t GDMA_SendData_Buffer[GDMA_BUFFER_SIZE];
volatile uint16_t GDMA_SendData_Size = 0;
...
void dbg_init(void)
{
    ...
    GDMA_InitStruct.GDMA_BufferSize          = 0; //pMCU_PPB->size_of_block;
    ...
    GDMA_InitStruct.GDMA_SourceAddr          = 0; //(uint32_t)(pMCU_PPB->start);
    GDMA_InitStruct.GDMA_ChannelPriority     = 7;
    GDMA_InitStruct.GDMA_SourceHandshake     = 0;
    GDMA_InitStruct.GDMA_Secure_En           = ENABLE;
    GDMA_InitStruct.GDMA_DestinationAddr = (uint32_t)(&(ZB_DBG_UART->RB_THR));
    GDMA_InitStruct.GDMA_DestHandshake   = GDMA_Handshake_UART2_TX;
//    void NSC_GDMA_CFG_REG_SET(uint8_t GDMA_ChannelNum);
//    NSC_GDMA_CFG_REG_SET(GDMA_CHANNEL_NUM);
    ...
    RamVectorTableUpdate(GDMA_Channel_VECTORn, GDMA_Channel_Handler);
}

void dbg_putchar(uint8_t c)
{
    GDMA_SendData_Buffer[GDMA_SendData_Size++] = c;
}

void dbg_flush(void)
{
    GDMA_SetSourceAddress(GDMA_Channel, (uint32_t)GDMA_SendData_Buffer);
    GDMA_SetBufferSize(GDMA_Channel, GDMA_SendData_Size);
...
{
    return (GDMA_SendData_Size > 0) ? 1 : 0;
}
...
int dbg_printf(const char *fmt, ...)
{
    ...
    {
        while (dbg_flush_busy());

        va_start(args, fmt);
        GDMA_SendData_Size = dbg_vsprintf((char *)GDMA_SendData_Buffer, fmt, args);
        va_end(args);

        dbg_flush();

        return GDMA_SendData_Size;
    }
}
