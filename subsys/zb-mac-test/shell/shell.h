...
// Section define
#define SECTION_SHELL_TEXT
//#define SECTION_SHELL_DATA
#define SECTION_SHELL_RODATA
#define SECTION_SHELL_BSS
...
#define CONFIG_SHELL_MAX_COMMAND_ARGS       32
...
typedef struct shell_command_entry_s
{
    ...
    shell_program_t
    shell_program;      /*!< the callback function association with the command string */
    const char *help_string;            /*!< the hel message for the command */
    unsigned int
    type; /*!< type of the command. type 1 for protothread execution*/
...
{
    const shell_command_entry_t
    *cmd_table; /*!< the pre-defined(compile time defined) table of command entrys */
    ...
    shell_reader_t
    shell_reader;    /*!< The function for the shell command to read a character from the terminal */
    shell_writer_t
    shell_writer;    /*!< The function for the shell command to write a character to the terminal */
    ...
    unsigned int
    cmd_list_size;    /*!< the size (number of command entrys) of the registerable command table */
    unsigned int shellbuf_sz;      /*!< the size (in bytes) of the command characters receive buffer */
    unsigned int
    buf_count;        /*!< the counter of the characters are received in the receive buffer */
    unsigned int
    buf_pos;          /*!< the position of the receive buffer, to indicate the cursor location
...
BOOL _shell_init(shell_command_t *pshell_cmd, shell_reader_t reader, shell_writer_t writer,
                 void *adapter);
...
void _shell_set_cmd_list(shell_command_t *pshell_cmd, shell_command_entry_t *cmd_list,
...
typedef struct cmd_shell_func_stubs_s
{
    ...
    BOOL (*shell_init)(shell_command_t *pshell_cmd, shell_reader_t reader, shell_writer_t writer,
                       void *adapter);
    ...
    void (*shell_set_cmd_list)(shell_command_t *pshell_cmd, shell_command_entry_t *cmd_list,
                               unsigned int list_size);
...
BOOL shell_register_pt(shell_program_t program, const char *cmd_string, const char *help_string);
