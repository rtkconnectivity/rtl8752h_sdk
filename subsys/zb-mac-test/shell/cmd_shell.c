...
{
    return cmd_shell_stubs.shell_register(&shell_cmd_hdl, program, cmd_string, help_string, 0);
}

BOOL shell_register_pt(shell_program_t program, const char *cmd_string, const char *help_string)
{
    return cmd_shell_stubs.shell_register(&shell_cmd_hdl, program, cmd_string, help_string, 1);
}
...
BOOL __attribute__((cmse_nonsecure_entry)) shell_init(shell_reader_t reader, shell_writer_t writer,
                                                      void *adapter)
{
...
void __attribute__((cmse_nonsecure_entry)) shell_set_cmd_table(const shell_command_entry_t
                                                               *cmd_table)
{
    shell_set_cmd_table(cmd_table);
}

void __attribute__((cmse_nonsecure_entry)) shell_set_cmd_list_ns(shell_command_entry_t *cmd_list,
                                                                 unsigned int list_size)
{
    shell_set_cmd_list(cmd_list, list_size);
}

BOOL __attribute__((cmse_nonsecure_entry)) shell_register_ns(shell_program_t program,
