...
typedef void (*before_tx_trigger_callback_t)(uint8_t pan_idx);
...
typedef struct mac_driver_rom_patch_s
{
    ...
    // rx
    uint8_t (*rx)(uint8_t *RxFIFO, uint32_t buffer_length);
...
typedef struct mac_driver_s
{
    ...
    int8_t txgain_tc_offset;
    int8_t txgain_k;

    uint16_t reserved[5];  // WARNING: DO NOT MODIFY SIZE OR CONTENT - ROM compatibility required!

    /* do not modify the members of this data struct listed above this line, in order to maintain
       the compatable with Bee4/rtl8752h/8852D ROM code !! */
    ...
    int8_t actual_tx_pwr;
    before_tx_trigger_callback_t mac_before_tx_trigger;
    // WARNING: DO NOT ADD/MODIFY MEMBERS ABOVE - MAINTAIN STRUCTURE LAYOUT!

    uint32_t reserved1[3];  // WARNING: DO NOT MODIFY SIZE OR CONTENT - Extension compatibility required!
...
typedef struct
{
    ...
    // software timer
#if !defined(CONFIG_MAC_SW_TIMER) || (CONFIG_MAC_SW_TIMER == 0)
    // Use BT timer as tick source
    void (*swtimer_init)(mac_timer_handle_t *ptmr_entities, uint8_t tmr_num);
#else
    // use MAC down counter timer as tick source
    bool (*swtimer_init)(uint32_t prescaler_tick_hz, uint8_t source_id);
#endif
    ...
    uint8_t (*trig_txn_at)(uint8_t ackreq, uint8_t secreq, uint8_t docca, uint32_t target_us);
    uint8_t (*trig_txenhack)(uint8_t early, uint8_t secreq);
    bool (*trig_upperenc)(void);
    void (*trig_upperdec)(void);
    // rx
    uint8_t (*rx)(uint8_t *RxFIFO, uint32_t buffer_length);
    uint8_t (*rxfifo)(uint8_t *RxFIFO, uint32_t buffer_length, uint8_t *MorePkt);
    // addr match filter
...
#endif
    uint8_t (*set_temp_comp)(int8_t offset);
    uint8_t (*set_gain_k)(int8_t k);
    uint8_t (*GetSrcShortAddrMatchEN)(uint8_t index);
    uint8_t (*GetSrcExtAddrMatchEN)(uint8_t index);
    uint32_t (*GetSrcExtMatch)(uint8_t index, bool high_bytes);
    void (*SetRxCrcErrPkt)(uint8_t en);
    uint8_t (*GetRxCrcErrPkt)(void);
    void (*SetNoCrcInTxNFIFO)(uint8_t en);
    uint8_t (*GetNoCrcInTxNFIFO)(void);
    uint8_t (*GetCcaMode)(void);
    uint32_t (*GetCcaCnt)(void);
    uint8_t (*GetRxFifoRdSel)(void);
    uint8_t (*GetImmAckPendingBit)(void);
    bool (*GetTxNCsma)(void);
    uint8_t (*GetMaxCSMABackoffs)(void);
    void (*SetCSMAMinBe)(uint8_t macMinBe);
    uint8_t (*GetCSMAMinBe)(void);
    void (*SetCSMAMaxBe)(uint8_t macMaxBe);
    uint8_t (*GetCSMAMaxBe)(void);
    uint8_t (*GetCurrentCcaReport)(void);
    uint8_t (*GetDatReqCmdPendingAck)(void);
    uint8_t (*GetPendingAck)(void);
    uint8_t (*GetEnhFrmPendEn)(void);
    uint16_t (*GetShortAddress)(void);
    uint8_t (*GetRxFrmSeqComp)(void);
    uint8_t *(*GetLongAddress)(void);
    int8_t (*GetTXPower)(void);
    uint8_t (*GetAddrMatchmode)(void);
    uint8_t (*GetEnhFrmPending)(void);
    uint8_t (*GetDataReqFrmPending)(void);
    uint32_t (*GetTxMStatus)(void);
    bool (*GetTxNTermedStatus)(void);
    bool (*GetTxEnhAckTermedStatus)(void);
    void *(*memset)(void *dst0, int Val, size_t length);
#if defined(CONFIG_MAC_SW_TIMER) && (CONFIG_MAC_SW_TIMER != 0)
    void (*mac_TimerPrescalEnableSet)(bool is_enable);
    bool (*mac_TimerPrescalEnableGet)(void);
    mac_status_t (*mac_TimerPrescalRateSet)(uint32_t tick_rate);
    uint32_t (*mac_TimerPrescalRateGet)(void);
    mac_status_t (*mac_TimerEnableSet)(uint8_t timer_id, bool is_enable);
    bool (*mac_TimerEnableGet)(uint8_t timer_id);
    mac_status_t (*mac_TimerStart)(uint8_t timer_id, uint32_t timeout_us);
    mac_status_t (*mac_TimerStop)(uint8_t timer_id);
    uint32_t (*mac_TimerValueGet)(uint8_t timer_id);
    mac_status_t (*mac_TimerBusyWait)(uint8_t timer_id);
#endif
} mac_adapter_t;
...
typedef struct
{
    ...
    uint8_t (*DualChannelIsEnabled)(void);
#endif  /* #if defined(DUAL_CHNL_EN) && (DUAL_CHNL_EN != 0) */
    uint8_t (*GetAddrMatchMode)(uint8_t pan_idx);
    uint8_t (*GetEnhFrmPending)(uint8_t pan_idx);
    uint8_t (*GetDataReqFrmPending)(uint8_t pan_idx);
    uint8_t (*GetRxFrmTypeFilter06)(uint8_t pan_idx);
    uint8_t (*GetRxFrmTypeFilter15)(uint8_t pan_idx);
    uint16_t (*GetShortAddress)(uint8_t pan_idx);
    uint8_t (*GetSrcShortAddrMatchPanIdx)(uint8_t index);
    uint8_t (*GetSrcExtAddrMatchPanIdx)(uint8_t index);
} mpan_stub_func_t;
#endif  /* #if defined(MULTI_PAN_EN) && (MULTI_PAN_EN != 0) */
...
/**
 * @brief ZB SysCall function pointer for inter-module communication (e.g., with BT)
 */
extern uint32_t (*ZB_SysCall)(uint32_t opcode, uint32_t param0, uint32_t param1, uint32_t param2);
...
/**
*
* @fn void mac_EDScanProcess(void)
*
* @brief ED Scan done interrupt handler
*
* @return None
*/
void mac_EDScanProcess(void);

/**
*
* @fn void mac_SoftwareTimer_IntrHandler(void)
*
* @brief MAC timer interrupt handler for software timer
*
* @return None
*/
void mac_SoftwareTimer_IntrHandler(void);
...
/**
*
* @fn uint8_t mac_SetTempComp(int8_t offset)
*
* @brief This function is used to set the RF TX power Temperature compensation.
*
* @param offset the TX gain offset
*
* @return The TX Gain value adjusted by Temperature compensation setting.
*
*/
static __inline uint8_t mac_SetTempComp(int8_t offset)
{
    return mac_adapter.set_temp_comp(offset);
}

/**
*
* @fn uint8_t mac_SetGainK(int8_t offset)
*
* @brief This function is used to set the RF TX power Gain K.
*
* @param k the TX gain K
*
* @return The TX Gain indx adjusted by Gain K.
*
*/
static __inline uint8_t mac_SetGainK(int8_t k)
{
    return mac_adapter.set_gain_k(k);
}
...
/**
 * @brief   Packs 4 bytes from a buffer into a 32-bit unsigned integer (little-endian).
 *
 * @param   p A pointer to the byte buffer.
 *
 * @return  The resulting 32-bit unsigned integer.
 *
 */
uint32_t pack_u32_from_bytes(const uint8_t *p);

/**
 * @brief   Safely gets the pointer to the high 32 bits of a source extended address match register.
 *
 * @param   index The index of the entry.
 *
 * @return  A volatile pointer to the register, or NULL if the index is invalid.
 *
 */
volatile uint32_t *mac_get_src_ext_match_h_ptr(uint8_t index);

/**
 * @brief   Safely gets the pointer to the low 32 bits of a source extended address match register.
 *
 * @param   index The index of the entry.
 *
 * @return  A volatile pointer to the register, or NULL if the index is invalid.
 *
 */
volatile uint32_t *mac_get_src_ext_match_l_ptr(uint8_t index);
...
/**
*
* @fn uint32_t mac_GetTxFrmLenStatus(void)
*
* @brief Get latese TX termed status
*
* @return the latest TX frm len error status:
*         bit0: GTS2 FIFO TX frm len error
*         bit1: GTS1 FIFO TX frm len error
*         bit2: Normal FIFO TX frm len error
*         bit3: Beacon FIFO Tx frm len error
*
*/
uint32_t mac_GetTxFrmLenStatus(void);
...
void mac_SetNoAckRsp(uint8_t en);

uint8_t mac_GetNoAckRsp(void);
...
uint8_t mac_RxFrameLength(void);

uint16_t mac_GetEnhAckDelay(void);
...
/**
*
* @fn uint8_t mac_RxFIFO(uint8_t *RxFIFO, uint32_t buffer_length, uint8_t *MorePkt)
*
* @brief Check RX FIFO Full status and read received packet from RX FIFO.
*
* @param *RxFIFO User data buffer pointer.
*
* @param buffer_length The size of the user data buffer in bytes.
*
...
*
* @return Rx status
* @retval MAC_STS_FAILURE There is no received packet or buffer is too small.
* @retval MAC_STS_SUCCESS Retrived one packet
*
*/
uint8_t mac_RxFIFO(uint8_t *RxFIFO, uint32_t buffer_length, uint8_t *MorePkt);
...
/**
*
* @fn bool mac_GetTxNTermedStatus(void)
*
* @brief to get the PHY arbitration term status of the latest packet TX
*
* @return the PHY term status
*
*/
...
/**
*
* @fn void mac_EDScan_begin(uint32_t scan_round)
*
* @brief trigger start of ED scan
*
* @param scan_round specified how many round for this ED scan, each round will scan
*        8 symbols time
*
* @return none
*
*/
void mac_EDScan_begin(uint32_t scan_round);

/**
*
* @fn void mac_EDScan_end(uint8_t *peak_value)
*
* @brief to query the ED scan result (peak value)
*
* @param peak_value to return the ED scan peak value
*
* @return none
*
*/
void mac_EDScan_end(uint8_t *peak_value);

/**
*
* @fn int8_t mac_edscan_level2dbm(uint8_t level)
*
* @brief to convert the ED scan/CCA level value to unit of dBm
*
* @param level the ED scan level (raw data) from hardware
*
* @return the converted value in unit of dBm
*
*/
...
/**
*
* @fn uint8_t mac_edscan_dbm2level(int8_t dbm)
*
* @brief to convert the ED scan/CCA value from unit of dBm
*         to the level for hardware
*
* @param dbm the ED value in unit of dBm
*
* @return the converted value in level for hardware setting
*
*/
...
/**
*
* @fn int8_t mac_GetTXPower_patch(void)
*
* @brief to query current TX power, in unit of dBm
*
* @return current TX power in unit of dBm
*
*/
int8_t mac_GetTXPower_patch(void);

/**
*
* @fn int8_t mac_SetTXPower_patch(int8_t tx_dbm)
*
* @brief to set the TX power, in unit of dBm
*
* @param tx_dbm the TX power, in unit of dBm, to set
*
* @return the actual TX power, in unit of dBm. The actual TX power
*         may not exectlty the same as the specified TX power due to
*         the Minimum or Maximum TX power RF configuration limit
*
*/
...
/**
*
* @fn uint8_t mac_GetCcaEDThreshold_patch(void)
*
* @brief to query current ED CCA threshold setting
*
* @return the ED CCA threshold setting (raw data for MAC hardware)
*
*/
uint8_t mac_GetCcaEDThreshold_patch(void);

void mac_PTA_Wrokaround(void);
...
uint8_t mpan_TrigTxNAtUS(bool_t AckReq, bool_t SecReq, bool_t DoCCA,
                         uint32_t TxTime, uint8_t pan_idx);

uint8_t mpan_RxAt(uint8_t aChannel, uint32_t aStart, uint32_t aDuration, uint8_t pan_idx);
...
/**
 * @brief Enable or disable the prescaler output tick.
 *
 * When disabled, no tick is delivered to timer channels. When enabled,
 * the prescaler generates ticks at the rate configured by
 * mac_TimerPrescalRateSet().
 *
 * @param[in] is_enable  @c true to enable, @c false to disable.
 *
 * @note This is a thin wrapper that delegates to @ref mac_adapter.
 *       Ensure @ref mac_adapter is initialized prior to calling.
 */
void mac_TimerPrescalEnableSet(bool is_enable);

/**
 * @brief Query whether the prescaler output tick is enabled.
 *
 * @retval true   Prescaler is enabled.
 * @retval false  Prescaler is disabled.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
bool mac_TimerPrescalEnableGet(void);

/**
 * @brief Configure the prescaler output tick rate (in Hz).
 *
 * The underlying implementation converts the requested tick frequency
 * to a hardware divider according to the MAC timer specification
 * (tick = 20 MHz / divider, with divider=0 -> 20 MHz).
 *
 * Typical valid range is 0.5 MHz ~ 2 Hz (subject to hardware limits).
 *
 * @param[in] tick_rate  Desired prescaler tick frequency in Hz.
 *
 * @retval MAC_STS_SUCCESS            Operation completed successfully.
 * @retval MAC_STS_INVALID_PARAMETER  The requested rate is out of range.
 * @retval MAC_STS_HW_LIMIT           Computed divider exceeds HW field width.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
mac_status_t mac_TimerPrescalRateSet(uint32_t tick_rate);
...
/**
 * @brief Read the current prescaler output tick rate (in Hz).
 *
 * @return Current prescaler tick frequency in Hz.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
uint32_t mac_TimerPrescalRateGet(void);
...
/**
 * @brief Enable or disable a specific MAC timer channel.
 *
 * @param[in] timer_id   Channel index (0..3).
 * @param[in] is_enable  @c true to enable, @c false to disable.
 *
 * @retval MAC_STS_SUCCESS            Operation completed successfully.
 * @retval MAC_STS_INVALID_PARAMETER  @p timer_id is out of range.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
mac_status_t mac_TimerEnableSet(uint8_t timer_id, bool is_enable);
...
/**
 * @brief Query whether a specific MAC timer channel is enabled.
 *
 * @param[in] timer_id   Channel index (0..3).
 * @retval true   Channel is enabled.
 * @retval false  Channel is disabled, or @p timer_id is invalid.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
bool mac_TimerEnableGet(uint8_t timer_id);
...
/**
 * @brief Start (or reload) a one-shot countdown on a timer channel.
 *
 * The countdown duration is a **relative** timeout (microseconds from now).
 * Channel must be enabled and the prescaler must be enabled beforehand.
 *
 * @param[in] timer_id     Channel index (0..3).
 * @param[in] timeout_us   Relative timeout in microseconds (must be > 0).
 *
 * @retval MAC_STS_SUCCESS            Operation completed successfully.
 * @retval MAC_STS_INVALID_PARAMETER  Invalid @p timer_id or @p timeout_us == 0.
 * @retval MAC_STS_FAILURE            Prescaler disabled or channel not enabled.
 * @retval MAC_STS_HW_LIMIT           Computed count exceeds 32-bit range.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
mac_status_t mac_TimerStart(uint8_t timer_id, uint32_t timeout_us);
...
/**
 * @brief Stop a running one-shot timer on the specified channel.
 *
 * Per spec, stopping does not generate an interrupt.
 *
 * @param[in] timer_id   Channel index (0..3).
 *
 * @retval MAC_STS_SUCCESS            Operation completed successfully.
 * @retval MAC_STS_INVALID_PARAMETER  Invalid @p timer_id.
 * @retval MAC_STS_FAILURE            Channel is disabled.
 *
 * @note Thin wrapper delegating to @ref mac_adapter.
 */
mac_status_t mac_TimerStop(uint8_t timer_id);
...
/**
 * @brief Read the current down-counter value of a channel (v0.4+).
 *
 * @details Returns the instantaneous 32-bit value from ZB_TMRx_READ. If the
 *          channel is idle/stopped it reads as 0; when disabled mid-run, HW
 *          retains the last value for firmware to read back.
 *
 * @param[in] timer_id 0..3
 * @return  current value (LSB-first). Returns 0 on invalid id.
 */
uint32_t mac_TimerValueGet(uint8_t timer_id);

/**
 * @brief Busy-wait until the specified channel reaches zero.
 * @note  Intended for short debug waits; prefer IRQ-driven completion.
 * @param[in] timer_id 0..3
 * @retval MAC_STS_SUCCESS            completed
 * @retval MAC_STS_INVALID_PARAMETER  bad id
 */
mac_status_t mac_TimerBusyWait(uint8_t timer_id);
#endif  //#if defined(CONFIG_MAC_SW_TIMER) && (CONFIG_MAC_SW_TIMER != 0)
